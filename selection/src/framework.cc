                             ...
                             const bool ismc)
{
    ...
    std::vector<CutFn<MatchedInteraction>> matched_cut_functions;
    for(const auto & cut : cuts)
    {
        ...
        else if(cut.get_string_field("type") == "matched")
        {
            std::string cut_name = "matched_" + name;
            std::vector<double> params;
            if(cut.has_field("parameters"))
                params = cut.get_double_vector("parameters");
            auto factory = CutFactoryRegistry<MatchedInteraction>::instance().get(cut_name);
            if(invert)
            {
                auto fn = factory(params);
                matched_cut_functions.push_back([fn](const MatchedInteraction & m) { return !fn(m); });
            }
            else
                matched_cut_functions.push_back(factory(params));
        }
        else
        {
    ...
    auto matched_cut = matched_cut_functions.empty()
        ? std::optional<CutFn<MatchedInteraction>>(std::nullopt)
        : std::optional<CutFn<MatchedInteraction>>([matched_cut_functions](const MatchedInteraction & m) -> bool {
            return std::all_of(matched_cut_functions.begin(), matched_cut_functions.end(),
                               [&m](auto & f) { return f(m); });
        });
    ...
    if(exec_mode == Mode::True)
    {
        ...
        else if(var_type == "matched")
        {
            var_name = "matched_" + var_name;
            auto factory = VarFactoryRegistry<MatchedInteraction>::instance().get(var_name);
            auto var_fn = factory(varPars);
            
            // For matched variables in true mode, we iterate over true interactions
            // and find their matched reco interactions
            auto matched_multivar = ana::SpillMultiVar([var_fn, true_cut, reco_cut_functions, 
                                                         event_cut, mctruth_cut, ismc, matched_cut]
                                                        (const caf::Proxy<caf::StandardRecord> * sr) -> std::vector<double>
            {
                ...
                // Iterate over true interactions
                for(auto const& truth_int : sr->dlp_true)
                {
                    // Find matched reco interaction
                    const RType* reco_int = nullptr;
                    if(truth_int.match_ids.size() > 0)
                    {
                        size_t reco_match = (size_t)truth_int.match_ids[0];
                        if(reco_match < sr->dlp.size())
                        {
                            reco_int = &sr->dlp[reco_match];
                        }
                    }
    ...
    else if(exec_mode == Mode::Reco)
    {
        ...
        else if(var_type == "matched")
        {
            ...
            // For matched variables, we build a custom lambda that iterates over reco
            // and evaluates the matched variable when truth is available
            auto matched_multivar = ana::SpillMultiVar([var_fn, reco_cut, true_cut_functions, 
                                                         event_cut, mctruth_cut, ismc, matched_cut]
                                                        (const caf::Proxy<caf::StandardRecord> * sr) -> std::vector<double>
            {
                std::vector<double> values;
                ...
                // Iterate over reco interactions
                for(size_t i = 0; i < sr->dlp.size(); ++i)
                {
                    auto const& reco_int = sr->dlp[i];
                    
                    // Find matched true interaction
                    const TType* truth_int = nullptr;
                    if(reco_int.match_ids.size() > 0)
                    {
                        size_t truth_match = (size_t)reco_int.match_ids[0];
                        if(truth_match < sr->dlp_true.size())
                        {
                            truth_int = &sr->dlp_true[truth_match];
                        }
                    }
                    
                    // Apply reco cuts and optional matched cuts
                    if(reco_cut(reco_int) && (!matched_cut || (truth_int && (*matched_cut)(MatchedInteraction{reco_int, truth_int}))))
                    {
                        values.push_back(var_fn(MatchedInteraction{reco_int, truth_int}));
                    }
                }
...
// matched_cut (optional) receives a MatchedInteraction bundling both the
// primary interaction and its matched partner, enabling cuts that inspect
// both reco and truth simultaneously.
...
ana::SpillMultiVar spill_multivar_helper(
    ...
    const bool ismc,
    const std::optional<CutFn<MatchedInteraction>> & matched_cut
)
{
            ...
            for(auto const& i : sr->dlp_true)
            {
                ...
                if constexpr(std::is_same_v<VarOn, RType>)
                {
                    if(cuts(i) && (!comps || (match_id != kNoMatch && (*comps)(sr->dlp[match_id])))
                       ...
                       && (!matched_cut || (match_id != kNoMatch && (*matched_cut)(MatchedInteraction{sr->dlp[match_id], &i}))))
                    {
            ...
            for(auto const& i : sr->dlp)
            {
                ...
                if constexpr(std::is_same_v<VarOn, TType>)
                {
                    if(cuts(i) && (!comps || (match_id != kNoMatch && (*comps)(sr->dlp_true[match_id])) || !ismc)
                        ...
                        && (!matched_cut || (*matched_cut)(MatchedInteraction{i, ismc && match_id != kNoMatch ? &sr->dlp_true[match_id] : nullptr})))
                    {
...
template class Registry<CutFactory<MatchedInteraction>>;
...
template class Registry<VarFactory<MatchedInteraction>>;
