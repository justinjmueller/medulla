...
def main(
    ...
    if test_job:
        ...
        launch_jobsub(project_dir, experiment, njobs=1, branch=branch, memory=memory, disk=disk, lifetime=lifetime, relaunch_missing=relaunch_missing)

    # If the user requested to launch jobs, do so.
    if launch_jobs is not None:
        if not project_exists:
            raise FileNotFoundError(f"Project database {project_dir / 'project.db'} does not exist. Please create a new project first.")
        launch_jobsub(project_dir, experiment, njobs=launch_jobs, branch=branch, memory=memory, disk=disk, lifetime=lifetime, relaunch_missing=relaunch_missing)
